...
def get_every_course_a_student_is_in(request, student_id):
    items = [x for x in db.find({
        "students_in_course.id": student_id},
        {'_id': 0}
    )]
...
@api_view(['GET'])
def get_a_course_a_student_is_in_filtered_by_student(request, student_id, course_name):
    items = [x for x in db.find(
        {
            "students_in_course.id": student_id,
            "course_name": course_name
        ...
        {"students_in_course": {
            "$elemMatch": {"id": student_id}}
         ,"course_name": 1, "_id": 0, "group": 1, "students_in_course": 1}
    )]
